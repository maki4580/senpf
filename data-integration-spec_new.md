...
IF-W01の `x_CameraHealth` 計測を生む前提経路(旧F1)。レビュー反映版タブで「ONVIF/ICMP死活確認」エッジとして図示済み。
...
「検知→通知→映像確認」動線(旧F2→F3→F4)の中核。レビュー反映版タブで「通知 Webhook」エッジ(イベント/アラーム処理→他アセット)として図示済み。メール(SMTP)経路は引き続き図外。
...
オブジェクトストレージの「映像クリップ+メタデータ」を成立させる経路(旧F5(b)。D2改訂の「クリップ保存指示」)。レビュー反映版タブで「クリップ保存指示(exportClip)」「映像クリップ+サイドカーJSON(S3互換)」エッジとして図示済み。
...
レビュー反映版タブで追加されたノード「運用知識基盤」(VM1)と、その前後の破線エッジ「テレメトリ」2本。製品/自作の別・データ内容・APIが未定義のため、本書では経路の存在と制約のみ確定する → **未確定事項N10**。

- **IF-H17(集約)**: Otelインフラ→運用知識基盤。テレメトリ(メトリクス/ログ/トレース)を運用知識として蓄積・加工する経路と解釈(仮)。プロトコル未定
- **IF-H18(提供)**: 運用知識基盤→他アセット。蓄積した運用知識(テレメトリ由来)をアセットへ提供する経路と解釈(仮)。プロトコル未定
